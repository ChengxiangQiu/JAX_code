...
#########################################
### systemactially nominating key TFs ###
#########################################

# 	Focusing on A, TF is highly expressed in MNN cells relative to the left cells.
# 	Focusing on A and B, TF is highly expressed in MNN cells of B than MNN cells of A.
# 	Focusing on B, TF is highly expressed in MNN cells relative to the left cells.
...
mouse_tf = read.table(paste0(work_path, "Mus_musculus_TF.txt"), header=T, as.is=TRUE, sep="\t")
...
for(cnt in 1:nrow(edges)){
    ...
    if(sum(group_table >= 20) >= 2){
        ...
        ### key TF
        gene_use = intersect(rownames(obj_sub), mouse_tf)
        res_1 = NULL
        res_2 = NULL
        res_3 = NULL
...
saveRDS(res_all, paste0(work_path, system_i, ".keyTF.rds"))
write.csv(res_all, paste0(work_path, system_i, ".keyTF.csv"))
...
saveRDS(res_all, paste0(work_path, "Neuroectoderm", ".keyTF.rds"))
write.csv(res_all, paste0(work_path, "Neuroectoderm", ".keyTF.csv"))
...
for(i in 1:length(system_list)){
    ...
    res = readRDS(paste0(work_path, system_i, ".keyTF.rds"))
...
write.csv(result_out, paste0(work_path, "All", ".keyTF.csv"))



### filtering top 10 significant TFs only
...
write.csv(as.data.frame(result), paste0(work_path, "All", ".keyTF.simple.csv"))
